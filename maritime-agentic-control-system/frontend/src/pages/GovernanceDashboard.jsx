...
export default function GovernanceDashboard() {
  ...
  if (loading && agents.length === 0) {
    return <div style={{ color: 'white', padding: '20px' }}>Loading Governance Layer...</div>;
  }
      ...
      <div className="panel" style={{ padding: '0', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-light)' }}>
          ...
          <TabButton active={activeTab === 'approvals'} onClick={() => setActiveTab('approvals')}>
            Human Approvals {approvals.length > 0 && <span style={{ background: 'var(--accent-red)', color: 'white', padding: '2px 6px', borderRadius: '10px', fontSize: '0.75rem', marginLeft: '6px' }}>{approvals.length}</span>}
          </TabButton>
        ...
        {agents.map(a => (
          <tr key={a.id} style={{ borderBottom: '1px solid var(--border-light)' }}>
            <td style={{ padding: '12px 8px' }}>
              <div style={{ fontWeight: 'bold', color: 'white' }}>{a.agent_name}</div>
            ...
            <td style={{ padding: '12px 8px' }}>
              {a.status === 'ACTIVE' ? (
                <button 
                  ...
                  style={{ background: 'var(--accent-red)', color: 'white', border: 'none', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '0.75rem' }}>
                  Quarantine
                </button>
              ) : (
                <button 
                  onClick={() => onStatusChange(a.id, 'ACTIVE')}
                  style={{ background: 'var(--accent-emerald)', color: 'black', border: 'none', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 'bold' }}>
                  Enable
                </button>
              )}
            </td>
...
function PendingApprovals({ approvals, onApprove, onReject }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {approvals.map(a => (
        <div key={a.id} style={{ border: '1px solid var(--accent-amber)', borderRadius: '8px', padding: '16px', background: 'rgba(245, 158, 11, 0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <AlertTriangle color="var(--accent-amber)" size={20} />
              <strong style={{ color: 'white', fontSize: '1.1rem' }}>Approval Required: {a.agent_name}</strong>
            </div>
          ...
          <div style={{ marginBottom: '16px', display: 'flex', gap: '16px' }}>
            ...
            <div><span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Confidence:</span> <span style={{ color: 'white', fontWeight: 'bold' }}>{(a.confidence * 100).toFixed(1)}%</span></div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button 
              ...
              style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--accent-emerald)', color: 'black', fontWeight: 'bold', border: 'none', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer' }}>
