  ...
  return (
    <div className="glass-panel" style={{ padding: '24px' }}>
      <div className="section-header">
        ...
        <span className="status-badge" style={{ fontSize: '0.75rem', background: 'rgba(255, 184, 0, 0.15)', borderColor: 'var(--accent-amber)', color: 'var(--accent-amber)' }}>
          LIVE ESTIMATES
        </span>
      </div>
        ...
        {data.map((item, i) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ width: '100%', height: '10px', background: 'rgba(255,255,255,0.06)', borderRadius: '5px', overflow: 'hidden', position: 'relative' }}>
                <div style={{
                  width: `${widthPct}%`,
                  height: '100%',
              ...
              {isHovered && (
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                  Primary Factor: <strong style={{ color: '#fff' }}>{item.reason}</strong>
                </div>
              )}
            </div>
          );
